...
from pipeline.step2_gpumd import make_orthogonal
from pipeline.utils import move_spectra
...
def run(d):
    ...
    for item in pipeline:
        for d_func, d_args in item.items():
            print(f'start {d_func}')
            match d_func:
                #  step 1
                ...
                case 'make_orthogonal':
                    args = d_args.copy()
                    make_orthogonal(d, **args)
                case 'calculate_spectra':
                    args = d_args.copy()
                    calculate_spectra(d, **args)
                case 'move_spectra':
                    args = d_args.copy()
                    move_spectra(d, **args)
                case _:
